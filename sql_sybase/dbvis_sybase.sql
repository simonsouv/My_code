...
sp_help TRN_CPDF_DBF; --COREPL_REP DYN_AUDIT_REP
...
select M_RFG_TABLE_NAME,M_RFG_FORMULA,M_RFD_TABLE_NAME,M_RFD_FORMULA from RDBCNSTR_DBF where M_RFG_TABLE_NAME='TRN_PFLD_DBF'; and M_RFG_FORMULA like 'M_REF%';
...
--3.1 queries
select * from CR_CTRY_SST_AUD;
select M_IDENTITY, str_replace(M_LABEL,' ' ,'-') from DAT_ECH_DBF where M_LABEL like '%1WEEK MODFOL%';
select M_IDENTITY,str_replace(T1.M_LABEL,' ' ,'-'), T1.M_GENERAT,T1.M_DIMENSION from DAT_ECH_DBF T1  where (T1.M_LABEL like '%1OD%' and T1.M_GENERAT =  0.); and T1.M_DIMENSION =  0. and T1.M_GENERAT =  0.);
update DAT_ECH_DBF set M_LABEL='1OD' where M_IDENTITY=25;
update DAT_ECH_DBF set M_LABEL='1Y MODFOL NY' where M_IDENTITY=1148;
update DAT_ECH_DBF set M_LABEL='1WEEK MODFOL' where M_IDENTITY=161;
select db_name(),count(*) from TRN_CPDF_DBF where M_ISSUER='Y';
...
select count(*) from TRN_HDR_DBF T1 left join TRN_HDRO_DBF T2 on T1.M_NB = T2.M_NB where T2.M_NB is null;
select @@servername,db_name(),count(*) from TRN_HDR_DBF where M_INSTRUMENT not in (select M_REFERENCE from TRN_PLIN_DBF) and M_PURGE_REF=0;
select * from CFGT_TMPL_DBF;
update USERNDX_DBF set M_DBFNAME='_INDEX' where M_IDENTITY=242;
select * from TRN_HDR_DBF where M_NB=2680265;
select * from TRN_PLIN_DBF where M_LABEL like 'EURIBOR 3M';
select * from TYPOLOGY_DBF;
select count(*) from TRN_HDR_DBF where M_TRN_FMLY='IRD' and M_TRN_GRP='CF' and M_TRN_GTYPE=4;
select T2.M_FAMILY,T2.M_FMLY_LBL, T2.M_LABEL, count(*) from TRN_HDR_DBF T1 join TRN_PLIN_DBF T2 on T1.M_INSTRUMENT = T2.M_REFERENCE where T1.M_TRN_FMLY='IRD' and T1.M_TRN_GRP='CF' and T1.M_TRN_GTYPE=4 and T2.M_LABEL like 'EURIBOR%' group by T2.M_FAMILY,T2.M_FMLY_LBL, T2.M_LABEL;
select count(*) from TRN_HDR_DBF T1 left join TRN_HDRO_DBF T2 on T1.M_NB=T2.M_NB where T2.M_NB is null;
select count(*) from TRN_HDR_DBF where M_INSTRUMENT not in (select M_REFERENCE from TRN_PLIN_DBF) and M_PURGE_REF=0;
